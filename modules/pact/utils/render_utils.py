    ...
    for j, (extr, intr) in tqdm(
        ...
        if isinstance(sample, MeshExtractResult):
            ...
            if not return_tensor:
                ...
                rets["normal"].append(
                    torch.clip(
                        res["normal"].detach().permute(1, 2, 0) * 255, 0, 255
                    ).to(torch.uint8)
                )
            ...
            if not return_tensor:
                ...
                rets["color"].append(
                    torch.clip(res["color"].detach().permute(1, 2, 0) * 255, 0, 255).to(
                        torch.uint8
                    )
                )
            ...
            if "percent_depth" in res:
                if not return_tensor:
                    ...
                    rets["depth"].append(res["percent_depth"].detach())
            elif "depth" in res:
                if not return_tensor:
                    rets["depth"].append(res["depth"].detach().cpu().numpy())
                else:
                    rets["depth"].append(res["depth"].detach())
            else:
...
def render_video(
    ...
    yaws = torch.linspace(0, 2 * 3.1415, num_frames) + 3.1415

...
def _fill_holes(
    ...
    for i in range(num_views):
        y, p = sphere_hammersley_sequence(
            i, num_views
    ...
    if debug:
        ...
        vis_colors[remove_face_indices.cpu().numpy()] = [
            255,
            0,
            255,
        ]  # Magenta for removed by mincut
...
def bake_texture(
    ...
    views = [
        utils3d.torch.extrinsics_to_view(torch.tensor(extr).cuda())
        for extr in extrinsics
    ]
    projections = [
        utils3d.torch.intrinsics_to_perspective(torch.tensor(intr).cuda(), near, far)
    ...
    if mode == "fast":
        ...
        for observation, view, projection in tqdm(
            ...
            with torch.no_grad():
                ...
                uv_map = (
                    rast["uv"][0].detach().flip(0)
                )  # Flip Y to match texture convention
                mask = (
                    rast["mask"][0].detach().bool() & masks[0]
                )  # Only use valid mask pixels
        ...
        rastctx = utils3d.torch.RastContext(backend="cuda")
        observations = [
            observations.flip(0) for observations in observations
        ...
        for observation, view, projection in tqdm(
            ...
            with torch.no_grad():
                ...
                _uv_dr.append(
                    rast["uv_dr"].detach()
                )  # Gradient information for differentiable rendering
...
def sample_pcd_from_mesh(
    ...
    pcd, _ = trimesh.sample.sample_surface_even(mesh, count=count)
...
def to_glb(
    ...
    verbose: bool = True,
    textured: bool = True,
    transform_zup_to_yup: bool = True,
) -> trimesh.Trimesh:
    """
    ...
    vertices, faces = postprocess_mesh(
        ...
        fill_holes_max_hole_nbe=int(
            250 * np.sqrt(1 - simplify)
        ),  # Scale hole size by mesh complexity
...
def simplify_gs(
    ...
    num_target = int(
        (1 - simplify) * _zeta.shape[0]
    )  # Target number of Gaussians after simplification

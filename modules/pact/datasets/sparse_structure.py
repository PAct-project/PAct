...
import utils3d
from .components import StandardDatasetBase, StandardDatasetBase_Part
...
class SparseStructure(StandardDatasetBase):
    ...
    def get_instance(self, root, instance):
        position = utils3d.io.read_ply(os.path.join(root, "voxels", f"{instance}.ply"))[
            0
        ]
        coords = ((torch.tensor(position) + 0.5) * self.resolution).int().contiguous()
        ss = torch.zeros(
            1, self.resolution, self.resolution, self.resolution, dtype=torch.long
        )
...
class SparseStructure_PartNet(StandardDatasetBase_Part):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(roots, split_info_json=split_info_json, is_test=is_test)

    def get_instance(self, instance_path):
        # position = utils3d.io.read_ply(os.path.join(root, 'voxels', f'{instance}.ply'))[0]
        full_position_path = os.path.join(
            instance_path, "trellis_part_preprocess/voxels_merged_fixed/full/mesh.ply"
        )
        position = utils3d.io.read_ply(full_position_path)[0]
        coords = ((torch.tensor(position) + 0.5) * self.resolution).int().contiguous()
        ss = torch.zeros(
            1, self.resolution, self.resolution, self.resolution, dtype=torch.long
        )
        ss[:, coords[:, 0], coords[:, 1], coords[:, 2]] = 1
        return {"ss": ss, "coords": coords, "position": torch.tensor(position)}
